...
const CustomNextArrow = (props: any) => {
  ...
  return (
    <button
      className="absolute top-1/2 -translate-y-1/2 right-2 sm:right-4 bg-gray-800/50 hover:bg-gray-800/80 transition-colors rounded-full p-1.5 sm:p-2 z-10"
      onClick={onClick}>
...
const CustomPrevArrow = (props: any) => {
  ...
  return (
    <button
      className="absolute top-1/2 -translate-y-1/2 left-2 sm:left-4 bg-gray-800/50 hover:bg-gray-800/80 transition-colors rounded-full p-1.5 sm:p-2 z-10"
      onClick={onClick}>
...
export default function PrintingServiceDetail() {
  ...
  const [options, setOptions] = useState<Record<string, string>>(() => {
    ...
    if (service && service.calculator) {
      service.calculator.options.forEach((opt: any) => {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className={`pt-24 sm:pt-32 pb-8 sm:pb-16 relative overflow-hidden bg-gradient-to-br ${service.color} text-white`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }}>
            <div className="w-10 h-10 sm:w-20 sm:h-20 bg-white/20 rounded-xl sm:rounded-2xl flex items-center justify-center mb-3 sm:mb-6">
              <Icon className="w-5 h-5 sm:w-12 sm:h-12" />
            </div>
            <h1 className="text-2xl sm:text-5xl lg:text-6xl font-bold mb-2 sm:mb-4 break-words">{service.title}</h1>
            <p className="text-sm sm:text-xl lg:text-2xl opacity-90">{service.subtitle}</p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-6 sm:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col lg:grid lg:grid-cols-3 gap-6 lg:gap-16">
            {/* Description + Slider — always first on mobile */}
            <div className="lg:col-span-2">
              <h2 className="text-xl sm:text-3xl font-bold text-gray-900 mb-3 sm:mb-4">Подробное описание</h2>
              <div className="text-sm sm:text-base text-gray-600 leading-relaxed mb-6 sm:mb-12">
                {service.fullDescription.split('\n\n').map((p: string, i: number) => <p key={i} className="mb-3 last:mb-0">{p}</p>)}
              </div>
              <div className="relative -mx-4 px-4 sm:mx-0 sm:px-0">
                <div className="shadow-xl sm:shadow-2xl rounded-xl sm:rounded-2xl overflow-hidden">
                  <Slider {...sliderSettings}>
                    {service.images.map((img: string, i: number) => (
                      <div key={i}>
                        <ImageWithFallback src={img} alt={`${service.title} image ${i + 1}`} className="w-full h-44 sm:h-80 object-cover" />
                      </div>
                    ))}
                  </Slider>
                </div>
            ...
            {/* Sidebar — second on mobile, right column on desktop */}
            <div className="lg:col-span-1">
            {!service.calculator && service.priceList && (
                <div className="bg-gray-50 rounded-xl sm:rounded-2xl p-4 sm:p-8 lg:sticky lg:top-28 shadow-lg">
                  <h3 className="text-lg sm:text-2xl font-bold text-gray-900 mb-3 sm:mb-6">Прайс-лист</h3>
                  <ul className="divide-y divide-gray-200 mb-4 sm:mb-6">
                    {service.priceList.map((p: { name: string; price: string }, i: number) => (
                      <li key={i} className="flex items-center justify-between gap-2 py-2 sm:py-3">
                        <span className="text-gray-700 text-xs sm:text-base">{p.name}</span>
                        <span className={`font-semibold whitespace-nowrap text-xs sm:text-base bg-gradient-to-r ${service.color} bg-clip-text text-transparent`}>{p.price}</span>
                      </li>
                    ))}
                  </ul>
                  <button
                    onClick={() => navigate(`/printing-services/order-confirmation?serviceTitle=${encodeURIComponent(service.title)}`)}
                    className={`w-full py-2.5 sm:py-3 bg-gradient-to-r ${service.color} text-white rounded-xl hover:shadow-xl transition-all font-semibold flex items-center justify-center gap-2 group text-sm sm:text-base`}>
                    Оставить заявку
            ...
            {service.calculator && (
                <div className="bg-gray-50 rounded-xl sm:rounded-2xl p-4 sm:p-8 lg:sticky lg:top-28 shadow-lg">
                  <h3 className="text-lg sm:text-2xl font-bold text-gray-900 mb-3 sm:mb-6">Калькулятор стоимости</h3>

                  <div className="space-y-3 sm:space-y-4 mb-4 sm:mb-6">
                    {service.calculator.options.map((opt: any) => (
                      <div key={opt.id}>
                        <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">{opt.name}</label>
                        {opt.type === 'radio' && (
                          <div className="flex flex-wrap gap-1.5 sm:gap-2">
                            {opt.values.map((val: string) => (
                              <button key={val} onClick={() => handleOptionChange(opt.id, val)} className={`px-2.5 py-1.5 sm:px-4 sm:py-2.5 rounded-lg text-[11px] sm:text-sm transition whitespace-nowrap ${options[opt.id] === val ? `bg-gradient-to-r ${service.color} text-white shadow` : 'bg-white border border-gray-300 hover:bg-gray-100'}`}>
                                {val}
                              </button>
                            ))}
                          </div>
                        ...
                        {opt.type === 'select' && (
                          <select onChange={(e) => handleOptionChange(opt.id, e.target.value)} value={options[opt.id]} className="w-full border border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 py-2 sm:py-2.5 px-2 sm:px-3 text-xs sm:text-base">
                            {opt.values.map((val: string) => <option key={val}>{val}</option>)}
                          </select>
                        )}
                      </div>
                    ))}
                  ...
                  {service.allowImageUpload && (
                    <div className="mb-4 sm:mb-6">
                      <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">Ваш дизайн</label>
                      <input type="file" accept="image/*" ref={fileInputRef} onChange={handleImageUpload} className="hidden" />
                      <button
                        onClick={() => fileInputRef.current?.click()}
                        className="w-full flex items-center justify-center gap-2 px-3 py-2.5 sm:py-3 border-2 border-dashed border-gray-300 rounded-lg text-xs sm:text-sm text-gray-600 hover:bg-gray-100 transition">
                        <Upload className="w-4 h-4 sm:w-5 sm:h-5" />
                        <span>{uploadedImage ? "Изменить фото" : "Загрузить фото"}</span>
                      </button>
                      {uploadedImage && <img src={uploadedImage} alt="Uploaded design" className="mt-3 rounded-lg shadow-md max-h-32 sm:max-h-none object-cover" />}
                    </div>
                  )}

                  <div className="mb-4 sm:mb-6">
                    <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1.5 sm:mb-2">{service.id === 'large-format' ? 'Площадь (м²)' : 'Количество'}</label>
                    <div className="flex items-center gap-2">
                      <button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="p-1.5 sm:p-2 rounded-full bg-gray-200 hover:bg-gray-300"><Minus className="w-3.5 h-3.5 sm:w-4 sm:h-4" /></button>
                      <input type="number" value={quantity} onChange={(e) => setQuantity(parseInt(e.target.value) || 1)} className="w-16 sm:w-20 text-center border-0 bg-transparent text-base sm:text-lg font-semibold focus:ring-0" />
                      <button onClick={() => setQuantity(q => q + 1)} className="p-1.5 sm:p-2 rounded-full bg-gray-200 hover:bg-gray-300"><Plus className="w-3.5 h-3.5 sm:w-4 sm:h-4" /></button>
                    </div>
                  </div>

                  <div className="text-center mb-4 sm:mb-6">
                    <p className="text-xs sm:text-sm text-gray-500">Примерная стоимость</p>
                  ...
                  <button onClick={handleOrder} className={`w-full py-2.5 sm:py-3 bg-gradient-to-r ${service.color} text-white rounded-xl hover:shadow-xl transition-all font-semibold flex items-center justify-center gap-2 group text-sm sm:text-base`}>
                    Заказать
                    <ArrowRight className="w-4 h-4 sm:w-5 sm:h-5 group-hover:translate-x-1 transition-transform" />
                  </button>
                </div>
